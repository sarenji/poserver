...
var INCLUDING = false;
var EXCLUDING = true;
var challenge_cups = ["Challenge Cup", "1v1 CC"];
...
var rubysapphire200Banlist;
tier_checker.add_new_check(INCLUDING, ["RS 200"], function rubysapphire200Check(src) {
    var poke;
    if (typeof rubysapphire200Banlist === 'undefined') {
        var pokes = {
        ...
        rubysapphire200Banlist = {};
        for (poke in pokes) {
            var pokeNum = sys.pokeNum(poke);
            if (!pokeNum) {
                sys.sendAll("DEBUG: " + poke + " is undefined in RS 200.");
                continue;
            }
            rubysapphire200Banlist[pokeNum] = [];
            for (var k = 0; k < pokes[poke].length; ++k) {
                var moveNum = sys.moveNum(pokes[poke][k]);
                if (!moveNum) {

                    sys.sendAll("DEBUG: " + pokes[poke][k] + " and " + poke + " produces a fatal error.");
                    continue;
                }
                rubysapphire200Banlist[pokeNum].push(moveNum);
            }
        }

    }
    var valid = true;
    var debug = function(msg) { if (false) { sys.sendAll(msg); }};
    for (var i = 0; i < 6; ++i) {
        poke = sys.teamPoke(src, i);
        if (poke !== 0 && !rubysapphire200Banlist.hasOwnProperty(poke)) {
            sys.sendAll("DEBUG: " + sys.pokemon(poke) + " has incorrect ban for RS 200 in tiers.yml.");
            sendMessage(src, sys.pokemon(poke) + " is not allowed in RS 200!");
            valid = false;
            continue;
        }
        if (poke === 0) continue;
        debug("Banned moves:" + rubysapphire200Banlist[poke].join(", "));
        for (var j = 0; j < 4; ++j) {
            var move = sys.teamPokeMove(src, i, j);
            if (rubysapphire200Banlist[poke].indexOf(move) >= 0) {
                sendMessage(src, sys.pokemon(poke) + " cannot have the move " + sys.move(move) + " in RS 200!");
                valid = false;
            }
        }
    }
